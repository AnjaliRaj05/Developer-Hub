...
let Dashboard = () => {
   ...
   const getProfile = async() =>{
    ...
    if (status==200){
      ...
      setFollowers(data.profile.followers.length);
    setFollowing(data.profile.following.length)
    }
    ;
 ...
 setLoading(false);
  ...
  let clickDeleteExperience = async(experienceId) => {
    const {data} = await axios.delete(`http://localhost:5001/api/profiles/experience/${experienceId}`,{
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("devroom")}`,
      },
  ...
  let clickDeleteEducation = async(educationId) => {
   const { data } = await axios.delete(
     ` http://localhost:5001/api/profiles/education/${educationId}`,
     {
       headers: {
         "Content-Type": "application/json",
         Authorization: `Bearer ${localStorage.getItem("devroom")}`,
            ...
            <div className="container">
              ...
              <section>
                <div className="container">
                  <div className="row">
                    <div className="col">
                    <p className="btn btn-primary text-white btn-sm me-2">Followers - {followers}</p>
                    <p className="btn btn-primary text-white btn-sm ">Following - {following}</p> 
                    </div>
                  </div>
                </div>
              </section>
